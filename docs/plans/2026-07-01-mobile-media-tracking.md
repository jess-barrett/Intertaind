...
- **Deep-link handling:** DONE (structure) in Task 0.2 — `media/[id]` is a SHARED route inside the tab navigator (`(tabs)/(index,explore)/media/[id].tsx`), extrapolated into both per-tab Stacks. A cold-start / deep link to `/media/<id>` resolves under `(tabs)/(index)` (the `(index)` group's anchor is `index.tsx`), so the tab navigator + Trending home mount BENEATH the pushed detail and the custom navbar is present immediately. STILL TODO: signed-out deep links must route through the login gate and then restore the intended route (`RootNavigator` currently `router.replace("/(auth)/login")` on `!session`, dropping the target — add preserve+restore of the intended href).
