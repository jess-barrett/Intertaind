...
- **⚠️ Username uniqueness data-integrity (BACKEND — verify before relying on it at scale):** The client-side uniqueness pre-check in `useCreateProfileMutation` (and web's `createInitialProfile`) is only best-effort — it is defeated by (a) concurrency/TOCTOU (two submits both pass the check then both insert) and (b) RLS: the uniqueness SELECT runs as the user, and `profiles_select_public_or_owner` (migration 004) hides other users' *private* profiles, so a name held by a private profile reads as available. The **only** authoritative arbiter is a database **case-insensitive unique constraint** on `profiles.username` (a `UNIQUE INDEX ON profiles (lower(username))` or a `citext` column). This could NOT be verified from the repo — `profiles` is defined in the remote Supabase base schema, not in `supabase/migrations/`. **Action: confirm the constraint exists (Supabase dashboard → Table editor → profiles indexes, or `\d profiles`); if missing, add it as a migration.** The mobile insert now remaps Postgres `23505` → "Username is already taken." so the constraint (if present) surfaces cleanly. Without the constraint, duplicate usernames are possible at scale.
- **Apply the same `ilike` wildcard escape + `23505` remap to web** `apps/web/src/app/actions/profile.ts` (`createInitialProfile`/`updateProfile`) — it has the identical `.ilike("username", name)` bug (a `_` in a username is a LIKE wildcard → false "taken") and raw-error surfacing. Fix shared-side for consistency.
